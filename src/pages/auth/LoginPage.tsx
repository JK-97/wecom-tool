...
export default function LoginPage() {
  ...
  const visibleErrorPresentation = useMemo(() => {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 p-4">
      ...
      >
        ...
        <CardContent className="space-y-4">
          ...
          {visibleErrorPresentation ? (
            <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              <div className="font-medium">{visibleErrorPresentation.title}</div>
              {visibleErrorPresentation.description ? (
                <div className="mt-1 text-xs leading-5 text-amber-800">{visibleErrorPresentation.description}</div>
              ) : null}
            </div>
          ) : null}
          {visibleErrorPresentation ? (
