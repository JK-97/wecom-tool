...
function readRuntimeDiagnostics(): RuntimeDiagnostics {
  ...
  const windowWW =
    typeof window === "undefined"
      ? undefined
      : (window as Window & { ww?: Record<string, unknown> }).ww;
...
function mapRuntimeError(
  ...
  if (
    ...
    normalized.includes("checkjsapi")
  ) {
    return new JSSDKRuntimeError(
      "api_unsupported",
      "当前企业微信客户端不支持所需 JS-SDK 能力。",
      {
        stage,
        diagnostics,
        message,
  ...
  if (
    stage === "get_context" ||
    normalized.includes("getcontext") ||
...
async function fetchSignatureBundle(
  targetURL: string,
  ...
  const payload = await requestJSON<SignatureReply>(
    "/api/v1/wecom/js-sdk/signature",
    {
      method: "POST",
      body: JSON.stringify({
        url: normalizedURL,
...
async function ensureSupportedAPIs(): Promise<Record<string, boolean>> {
  ...
  if (typeof ww.checkJsApi !== "function") {
    throw new JSSDKRuntimeError(
      "api_unsupported",
      "当前企业微信客户端不支持 checkJsApi。",
      { stage: "check_js_api" },
    );
  }

  logJSSDKDiagnostic("check_js_api_start", {
    api_list: [...CHECK_JS_API_LIST],
  });
  try {
    const raw = await ww.checkJsApi({ jsApiList: [...CHECK_JS_API_LIST] });
    const checkResult = toUnknownRecord(
      (raw as { checkResult?: unknown }).checkResult,
    );
...
export async function resolveSidebarRuntimeContext(): Promise<SidebarRuntimeContext> {
  try {
    ...
    logJSSDKDiagnostic("get_context_done", {
      entry,
      raw_entry: context?.entry,
...
export async function sendTextToCurrentSession(
  ...
  if (
    !runtime.api_support.sendChatMessage ||
    typeof ww.sendChatMessage !== "function"
  ) {
    throw new JSSDKRuntimeError(
      "api_unsupported",
      "当前客户端不支持 sendChatMessage。",
    );
  }
...
export async function openWecomKfConversation(target: {
  ...
  const apiSupport = await ensureSupportedAPIs();
  if (
    !apiSupport.navigateToKfChat ||
    typeof ww.navigateToKfChat !== "function"
  ) {
    throw new JSSDKRuntimeError(
      "api_unsupported",
      "当前客户端不支持打开微信客服会话。",
...
export async function checkMainWebviewJSSDKRuntime(): Promise<MainWebviewJSSDKCheckResult> {
  ...
  try {
    ...
    const raw = await ww.checkJsApi({
      jsApiList: [...MAIN_WEBVIEW_CHECK_API_LIST],
    });
    const checkResult = toUnknownRecord(
      (raw as { checkResult?: unknown }).checkResult,
    );
