...
export async function getKFToolbarBootstrap(params: {
  ...
  const payload = await requestJSON<APIReply<KFToolbarBootstrap>>(`/api/v1/kf/toolbar/bootstrap?${search.toString()}`)
...
export async function regenerateKFToolbarSuggestions(input: {
  ...
  const payload = await requestJSON<APIReply<KFToolbarSuggestionBatch>>("/api/v1/kf/toolbar/suggestions/regenerate", {
    method: "POST",
    body: JSON.stringify({
      entry: input.entry || "",
...
export async function sendKFToolbarReplyFeedback(input: {
  ...
  await requestJSON<APIReply<{ success?: boolean }>>("/api/v1/kf/toolbar/reply-feedback", {
    method: "POST",
    body: JSON.stringify({
      open_kfid: input.open_kfid || "",
      external_userid: input.external_userid || "",
